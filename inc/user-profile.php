...
function ct_apex_add_social_profile_settings($user) {
	...
	<table class="form-table">
		<tr>
			<th><h3><?php _e('Social Profiles', 'apex'); ?></h3></th>
		</tr>
		<?php
		foreach($social_sites as $key => $social_site) { ?>
			<tr>
				<th>
					<?php if( $key == 'email' ) : ?>
						<label for="<?php echo $key; ?>-profile"><?php echo ucfirst($key); ?> <?php _e('Address:', 'apex'); ?></label>
					<?php else : ?>
						<label for="<?php echo $key; ?>-profile"><?php echo ucfirst($key); ?> <?php _e('Profile:', 'apex'); ?></label>
					<?php endif; ?>
				</th>
				<td>
					<?php if( $key == 'email' ) : ?>
...
add_action( 'show_user_profile', 'ct_apex_add_social_profile_settings' );
add_action( 'edit_user_profile', 'ct_apex_add_social_profile_settings' );

function ct_apex_save_social_profiles($user_id) {
...
add_action( 'personal_options_update', 'ct_apex_save_social_profiles' );
add_action( 'edit_user_profile_update', 'ct_apex_save_social_profiles' );