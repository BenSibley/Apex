...
if ( $categories ) {
	...
	echo '<p class="post-categories">';
		...
		foreach ( $categories as $category ) {
			...
			$output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", 'apex' ), $category->name ) ) . '">' . esc_html( $category->cat_name ) . '</a>' . $separator;
		}
